    ...
    with container.container():
        if i not in st.session_state['episode']['steps'].keys():
            ...
            if policy == 'Choose your own action':
                ...
                diagnosis, rating, stddev, index = potential_diagnoses_ratings_indices[
                    diagnosis_index]
                with c2:
                    ...
                    if isinstance(get_actor(actor_checkpoint), InterpretableDirichletActor):
                        ...
                        st.write(
                            '**average of all concentrations**: {:.1f}'.format(concentration.mean()))
                        st.write(
                            '**{} concentration**: {:.1f}'.format(diagnosis, concentration[index]))
                        ...
                        if 'context_attn_weights' in parameters_info.keys() and sort_by_attn:
                            def key(
                                x): return -parameters_info['context_attn_weights'][index, x[1]]
                        evidence_concentrations = sorted(
                            evidence_concentrations, key=key)
                        for ec in evidence_concentrations:
                            evidence = ("\"" + ec[0] + "\"") \
                                if ec[1] > 0 or observation['evidence_is_retrieved'] else 'the report'
                            attn_string = "" if 'context_attn_weights' not in parameters_info.keys() else \
                                ", attn={:.0f}%".format(
                                    parameters_info['context_attn_weights'][index, ec[1]].item() * 100)
                            st.write(
                                '- (vote={:.1f}{}) {}'.format(ec[2], attn_string, evidence))
                        ...
                        dist_stddevs = [Beta(a, b).stddev for a, b in zip(
                            concentrations1[index], concentrations0[index])]
                        evidence_concentrations = list(zip(
                            context_strings, range(len(context_strings)), concentrations1[index], concentrations0[index], dist_means, dist_stddevs))
                        ...
                        for ec in evidence_concentrations:
                            ...
                            attn_string = "" if 'context_attn_weights' not in parameters_info.keys() else \
                                ", attn={:.0f}%".format(
                                    parameters_info['context_attn_weights'][index, ec[1]].item() * 100)
                            st.write(
                                '- (vote={:.1f}%±{:.1f}{}) {}'.format(ec[4] * 100, ec[5] * 100, attn_string, evidence))
